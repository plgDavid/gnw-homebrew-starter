...
static void update() {
  ...
  snprintf(buffer, 1024, "FRAME=%d\nbuttons=%lx\nbat=%d%% %s\n\nA = BSOD\n<> = change demo\nPAUSE = reboot", frame, buttons_get(), bq24072_get_percent_filtered(), battery_state[bq24072_get_state()]);
  frame++;
  ...
  switch (mode) {
    ...
    case 1:
      for (int j = 0; j < 16; j++) {
        for (int i = 0; i < 16; i++) {
          c[0] = i + j * 16;
          lcd_draw_text_8x8(i * 8, j * 8, c, 0xffff);
    ...
    case 2:
      clear_screen();
      for (int i = 0; i < 100; i++)
        lcd_draw_text_8x8(rnd() % WIDTH - 50, rnd() % HEIGHT, "Hello world. This is some funny text!", rnd() % 65536);
    ...
    case 3:
      ...
      for (int j = 0; j < HEIGHT / 8; j++) {
        for (int i = 0; i < WIDTH / 8; i++) {
          c[0] = rnd() % 96 + 32;
          lcd_draw_text_8x8(i * 8, j * 8, c, rnd() % 65536);
    ...
    case 7:
      ...
      lcd_draw_text_8x8(50, 50, buffer, 0x0f0f);
      break;
  }
}
